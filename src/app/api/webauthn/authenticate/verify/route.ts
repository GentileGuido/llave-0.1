...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (verification.verified) {
      ...
      // Derive key from user ID and credential ID for consistency
      const keyMaterial = `${userId}:${credential.id}`;
      const derivedKey = crypto.pbkdf2Sync(keyMaterial, 'llave-salt', 100000, 32, 'sha256');
